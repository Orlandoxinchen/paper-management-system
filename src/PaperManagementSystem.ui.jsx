  ...
  useEffect(() => {
    ...
    if (savedUser) {
      try {
        ...
        localStorage.removeItem('paper_system_user');
      }
    }
      ...
      {loading && (
        <div className="fixed inset-0 bg-black bg-opacity-30 z-50 flex items-center justify-center">
          <div className="bg-white rounded-xl p-6 shadow-2xl flex items-center gap-3">
            ...
            <span className="text-slate-700 font-medium">处理中...</span>
          </div>
        </div>
      )}
        ...
        <div className="max-w-7xl mx-auto px-6 py-4">
          <div className="flex items-center justify-between">
            <div>
              ...
              {user && <p className="text-xs text-slate-400 mt-1">👤 {user.email}</p>}
            </div>
                ...
                <button onClick={async () => {
                  ...
                  try {
                    if (deleteConfirm.type === 'author') {
                      await supabase.from('authors').delete().eq('id', deleteConfirm.id);
                      setAuthors(authors.filter(a => a.id !== deleteConfirm.id));
                      addHistory('删除', '作者', `删除作者：${deleteConfirm.name}`);
                      notify('已删除');
                    } else {
                      await supabase.from('papers').delete().eq('id', deleteConfirm.id);
